...
var Lib = (function(){
	...
	var timer; // Переменная для интервалов
	var countCities; // чило количества найденных городов
	...
	function renderCurrentWeather (data) {
		...
		var temp = tempConvert(data.main.temp);
		var pressure = pressureConvert(data.main.pressure);
		var windDirection = windConvert(data.wind.deg);
		var image = new Image();
	...
	function getForecastWeather (id) {
		...
		request.onload = function () {
			...
			if (request.status >= 200 && request.status < 400) {
				...
				var forecastArray = createForecastArray(data);
				// Рендерим прогноз
	...
	function renderDay(arr, idx) {
		...
		for ( var i = 0; i < dayForecast.length; i++ ) {
			...
			var cels = tempConvert(dayForecast[i].weather.temp);
